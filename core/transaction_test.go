...
func TestTransactionMarshalling(t *testing.T) {
	...
	tr := NewTransaction(kp.Public, nil, []byte(utils.RandomString(utils.RandomInt(0, 1024*1024))))
...
func TestTransactionVerification(t *testing.T) {
	...
	kp := GenerateNewKeypair()
	tr := NewTransaction(kp.Public, nil, []byte(utils.RandomString(utils.RandomInt(0, 1024))))
...
func TestIncorrectTransactionSignatureVerification(t *testing.T) {
	...
	tr := NewTransaction(kp2.Public, nil, []byte(utils.RandomString(utils.RandomInt(0, 1024))))
	tr.Header.Nonce = tr.GenerateNonce(pow)
